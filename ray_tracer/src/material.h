...
public:
    ...
    virtual bool scatter(const Ray& ray_in, const HitRecord& record, Color& attenuation, Ray& scattered, RandomGen& random_gen) const = 0;
};
...
public:
    ...
    virtual bool scatter(const Ray& ray_in, const HitRecord& record, Color& attenuation, Ray& scattered, RandomGen& random_gen) const override
    {
#if 1
        // trace reflected ray from within sphere at normal, kissing last hit
        Vec3 scatter_direction = record.normal + random_unit_vector(random_gen);
#else
        // trace reflected ray from within hemisphere of normal
        Vec3 scatter_direction = random_in_hemisphere(record.normal(random_gen);
#endif
...
public:
    ...
    virtual bool scatter(const Ray& ray_in, const HitRecord& record, Color& attenuation, Ray& scattered, RandomGen& random_gen) const override
    {
        ...
        scattered   = Ray(record.hit_point, reflected + m_fuzziness * random_in_unit_sphere(random_gen));
        attenuation = m_albedo;
...
public:
    ...
    virtual bool scatter(const Ray& ray_in, const HitRecord& record, Color& attenuation, Ray& scattered, RandomGen& random_gen) const override
    {
        ...
        if (cannot_refract || reflectance(cos_theta, eta_in_over_eta_trans) > random_gen.next_double())
